...
export function getProjectCover(title: string): string | undefined {
  ...
  if (t.includes('nesoi')) return '/nesoi/final-prototype.webp';
  if (t.includes('falcon')) return '/falcon design system/image.webp';
  if (t.includes('onboarding') || t.includes('ditto') || t.includes('booking')) return '/ditto insurance/1.webp';
  if (t.includes('crm')) return '/CRM/image.webp';
  if (t.includes('kiosk')) return '/photos/image.webp';
  return undefined;
}
