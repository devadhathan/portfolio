...
import { Clock, Calendar, FolderKanban, ChevronRight, ChevronsRight } from 'lucide-react';
...
export function WidgetsPanel({ onProjectSelect }: WidgetsPanelProps) {
  ...
  return (
    <aside
      className={cn(
        'os-widgets-panel pointer-events-auto fixed bottom-3 right-3 top-16 z-[190] flex w-80 max-w-[calc(100vw-1rem)] flex-col overflow-hidden rounded-2xl border border-white/10 bg-black/45 shadow-[0_18px_48px_-28px_rgba(0,0,0,0.55)] backdrop-blur-2xl backdrop-saturate-150 transition-transform duration-300 ease-out dark:bg-black/55',
    ...
    >
      <div className="flex flex-shrink-0 items-center justify-between px-4 pb-2 pt-4">
        <h2 className="text-sm font-semibold text-white/95">Widgets</h2>
        <Button
          type="button"
          variant="ghost"
          size="icon"
          ...
          className="h-8 w-8 rounded-full text-white/80 shadow-none transition-colors hover:bg-white/10 hover:text-white"
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden px-4 pb-2 pt-1">
        <Card className="flex h-full flex-col border-0 bg-transparent shadow-none">
          <CardHeader className="flex-shrink-0 px-0 pb-2">
